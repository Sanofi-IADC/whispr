...
export class TagGroupController {
  ...
  @HttpCode(204)
  @UsePipes(new ValidationPipe({ whitelist: true }))
  async updateTagGroup(@Param('id') id: string, @Body() tagGroup: TagGroupInputType): Promise<void> {
  ...
  @HttpCode(204)
  @UsePipes(new ValidationPipe({ whitelist: true }))
  async replaceTagGroup(@Param('id') id: string, @Body() tagGroup: TagGroupInputType): Promise<void> {
